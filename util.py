...
from dataset import AVA_Comment_Dataset_bert, ava_comment_InstanceSample, AVA_Comment_Dataset
...
def get_jointplot(img_score, text_score):
    d = {'img_score': img_score, 'text_score': text_score}
    df = pd.DataFrame(d)

    sns.jointplot(x='img_score', y='text_score', data=df, alpha=0.5)
...
def create_data_part_crd(opt):
    train_csv_path = os.path.join(opt.path_to_save_csv, 'train.csv')
    test_csv_path = os.path.join(opt.path_to_save_csv, 'test.csv')

    train_ds = ava_comment_InstanceSample(train_csv_path, opt.path_to_images, k=opt.nce_k)
    test_ds = AVA_Comment_Dataset_bert(test_csv_path, opt.path_to_images, if_train=False)
    # test_ds = AVA_Comment_Dataset(test_csv_path, opt.path_to_images, if_train=False)

    train_loader = DataLoader(train_ds, batch_size=opt.batch_size, num_workers=opt.num_workers, shuffle=True)
    test_loader = DataLoader(test_ds, batch_size=opt.batch_size, num_workers=opt.num_workers, shuffle=False)

    return train_loader, test_loader, train_ds.df.shape[0]
...
class Balanced_l1_Loss(nn.Module):
    def __init__(self, device):
        super(Balanced_l1_Loss, self).__init__()
        self.weight = torch.tensor([25, 11.4, 4.56, 1.8, 1, 1.18, 2.16, 4.35, 10.1, 16.94]).type(torch.FloatTensor).to(device)
    ...
    def forward(self, p_target, p_estimate):
        ...
        diff = p_estimate - p_target
        # raw = torch.pow(torch.abs(cdf_diff), 2)
        balanced = torch.abs(diff) * self.weight
        samplewise_l2 = torch.mean(balanced)
        return samplewise_l2.mean()
